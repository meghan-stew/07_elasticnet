...
recipe(strength_gtex ~ .,
       data = weather_train) %>%
# dot says all other columns in dataset should be predictors. We also just give training set. 

  # Removing year, site, and weather outside of growing season  : hoping weather patterns can be a way of predicting stength_gtex for a future year. 

  #step_rm does same thing as dplyr select but in recipe function house. MAtches will be used to ID the columns with month name at end to apply step_remove
step_rm(year,site,
        matches("Jan|Feb|Mar|Apr|Nov|Dec")
        )

  # Normalizing: prior to decorrelate using PCA
